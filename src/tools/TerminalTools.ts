...
export class TerminalTools {
  ...
  async executeCommand(
    command: string,
    changeId?: string,
  ): Promise<{ output: string; status: "success" | "error" | "cancelled" }> {
    ...
    if (!riskAssessment.safe) {
      ...
      console.log(
        `Command execution blocked due to risk assessment: ${command}`,
      );
  ...
  assessRisk(command: string): {
    ...
    if (
      mediumRiskCommands.some((risk) => command.toLowerCase().includes(risk))
    ) {
      return {
        level: "medium",
        color: "#f1c40f",
  ...
  isCommandApproved(command: string): boolean {
    ...
    const approved =
      this.isInWhitelist(command) && !this.isInBlacklist(command);
