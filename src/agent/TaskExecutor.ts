...
class RiskAssessmentManager {
   ...
   */
  static assessRisk(command: string): {
  ...
  static getColorForSafetyIndicator(
...
export class TaskExecutor {
    ...
    try {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        try {
          ...
          if (!recovered) {
            throw new Error(
              `Task execution failed at step ${i}: ${(error as Error).message}`,
            );
          }
        }
      }
      ...
      return {
        ...
        endTime: new Date(),
        duration: Math.floor(
          (new Date().getTime() - executionContext.startTime.getTime()) / 1000,
        ),
      };
    } catch (error) {
  ...
  private async attemptRecovery(
    ...
    while (retries < maxRetries) {
      try {
        console.log(
          `Attempting recovery for step ${failedIndex} (retry #${retries + 1})`,
        );
  ...
  selectTools(taskDescription: string): string[] {
    ...
    if (
      taskDescription.toLowerCase().includes("terminal") ||
  ...
  manageContext(taskId: string, stepIndex: number, contextData: any): void {
    ...
    console.log(
      `Managing context for task ${taskId}, step ${stepIndex}:`,
      contextData,
    );
  }
  ...
  getCommandRiskAssessment(command: string): {
    level: "low" | "medium" | "high";
    color: string;
  } {
