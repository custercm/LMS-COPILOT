  ...
  private handleHelp(args?: string): void {
    if (args) {
      ...
      const command = this.getCommands().find(
        (cmd) => cmd.name === args || cmd.name === `/${args}`,
      );
      if (command) {
...
`;
  ...
  private handleInstall(args?: string): void {
    if (!args) {
      this.context.showError(
        "Please specify a package to install. Usage: /install <package_name>",
      );
      return;
    }
  ...
  private handleSearch(args?: string): void {
    if (!args) {
      this.context.showError(
        "Please specify a search query. Usage: /search <query>",
      );
      return;
    }
    ...
    if (args) {
      ...
      if (value) {
        this.context.sendCommand(
          "updateSetting",
          JSON.stringify({ setting: setting.trim(), value: value.trim() }),
        );
      } else {
